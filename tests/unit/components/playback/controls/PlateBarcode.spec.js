...
describe("PlateBarcode.vue", () => {
  ...
  test.each([
    ...
    ["ML二千万一千〇九", "error due to all unicode", null],
    ...
    ["MS2021172000", "header 'MS'", "MS2021172000"],
    ["ML2021172001", "valid kit ID '001'", "ML2021172001"],
    ["ML2021172002", "valid kit ID '002'", "ML2021172002"],
    ["ML2021172003", "valid kit ID '003'", "ML2021172003"],
    ["ML2021172004", "valid kit ID '004'", "ML2021172004"],
    ["ML9999172001", "year '9999'", "ML9999172001"],
    ["ML2021001144", "julian date '001'", "ML2021001144"],
    ["ML2021366144", "julian date '366'", "ML2021366144"],
  ])(
  ...
  test("Given that its in manual mode and a valid barcode has been entered and playback state is BUFFERING, When Playback State is mutated to BUFFERING, Then the text of the Barcode Input remains as the valid barcode instead of becoming blank", async () => {
    ...
    await wrapper.vm.$nextTick(); // wait for update
    expect(wrapper.vm.plate_barcode).toEqual("ML2022053000");
  });
