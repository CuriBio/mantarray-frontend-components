...
describe("PlateMapEditor.vue", () => {
  ...
  test.each([
    ...
    "Given that none of the wells are selected, When user clicks the Row button, Then wells corresponding to that row is seleced with outline of 4px;",
    async (string_name_of_button, array_of_well_indices, selector_str) => {
      ...
      const row_selector = wrapper.find(selector_str);

      await row_selector.trigger("click");
